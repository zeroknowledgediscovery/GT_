...
\cgather{
\textit{I-complexity: } N/(N^2) = 1/N \\
\textit{Z-complexity: } 1  
}
...
% \subsection*{Measures of Reslience} In adition to stability which investigates the response of a system to input perturbations, we consider the resilience that refers to perturbations in system parameters.
...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=1, every node/.style={transform shape},font=\bf\sffamily\fontsize{8}{8}\selectfont]
    ...
    \def\COLORX{DodgerBlue2} % color for the x curve
    \def\COLORXdot{Red4} % color for the xdot curve
    ...
    yshift=\TextYSHIFT,
    %font=\fontsize{\TextFS}{\TextFS}\selectfont
    ] (V.label) at (V.north west) {
      {\large C.} Van der Pol 
      \cgathers{
      x - \mu\dot{x}\left(1 - x^2\right) = 2.5\cos(3t)\\
      \mu = \begin{bmatrix} 0.07 & 0.7  &  1.0 & 4.0. \end{bmatrix}}
    };
    
  \end{tikzpicture}
  \caption{Learning Processes Cross-talk in Complex Phenomena. Learning Processes Cross-talk in Complex Phenomena.Learning Processes Cross-talk in Complex Phenomena.Learning Processes Cross-talk in Complex Phenomena.Learning Processes Cross-talk in Complex Phenomena.}
\end{figure}
