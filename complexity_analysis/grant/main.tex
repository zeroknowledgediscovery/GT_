...
\input{Figures/figzipf}

\captionN{Zipf's Law holds in rela-world systems, but not in simulated ones, even  with significant effort and resources expended  to  replicate reality.}\label{fig1}
\end{figure}
...
strength of inferred predictive links between observables.
The justification of looking for such characterization in the distrbution of links strengths arises from the idea that an enumeration of the link strangths may be seen as a specification of the state  the complex system, and it can be shown that existence of the Zipf's law amounts to the existence of critical temperature below which the system ``freezes'', and above which thermodynamic quantities diverge~\cite{mora2011biological}.


As shown in Fig~\ref{fig1}, our preliminary studies corroborates this hypothesis, at least with the systems under consideration.
...
Design  of non-trivial complexity measures for the systems of interest poses the challenge that our systems are obviously not simple: theyhave many varaibles, componenets and dependencies. Simply specifying the size of the system is not discriminative enough, and is potentially misleading. We plan to develop measures that capture the complexity of information processing within the system, $e.g.$, the number of intrinsic states that the system has for it to manifest the observed behavior.

We plan to accomplish this by relating complexity to the number of models we vcan infer normalized by the maximum number of possible models,  and the average number of states in these set of models.
We call teh first measure I-complexity, and the second Z-complexity.
I-complexity really is the connectance that appears in May's formalism: what is the probability that two varaibles or entities interact. And Z-complexity captures the complexity of the dependence. 
