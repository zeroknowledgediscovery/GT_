...
3. &\large$\mu_\omega$ & \cellcolor{\COLD} Frequency-like & Generalization of frequecny domain tools and measures to non-linear multi-scale spatio-temporal system of systems with categorical and ordinal variables. \\\hline
4. &\large$\mu_\E$ & \cellcolor{\COLE} External Influence & Estimate the pssibility having an  open vs a closed system.\\\hline
5. &\mnp{.8in}{\centering \vskip .4em {\large$\partial \mu_i / \partial t$} \\ $ i= \C, \S, \omega, \E$\vskip 1.5em } & \cellcolor{\COLF} Evolution \&  Influence & How the measures of dynamical properties  evolve in time characterizes the evolution of  emergent rules and drivers, indicative of the rate of information generation either within, or via influence import.\\\hline
\end{tabular}
\end{table}
%####################################
%####################################
...
%Stability is well-understood  in the study of dynamical systems.
...
% For decades, physicists have hoped that the emergent, collective phenomena of life | both at the modelcular and social levels |   could be captured using ideas from statistical mechanics. For example, the stationary states of biological systems,   have a subtle structure, neither ``frozen'' into a well ordered crystal, nor chaotic and disordered like a gas.
...
%This allows us to think about the probability $P(\q)$ of finding the system in that state, and use standard statistical mechanics to write down the   Boltzmann distribution:
% \cgather{
% P(\q) = \frac{1}{Z} e^{E(\q)/k_BT}
%   }
% where $k_B$  is Boltzmann's constant and $Z$ the partition function. Without loss of generality, we can set the temperature $k_B T = 1,Z=1$,  leading  to the  definition of
% the energy as:
% \cgather{
%   E(\q)= -\log P(\q )
% }
% Using well-understood arguments  that equate density of states (number of states within a small energey braket)  to the exponential of the entropy $S$, we can eventually work out that Zipf's Law implies:
% \cgather{
% S = E/\alpha + \cdots (\textrm{subextensive terms})
% }

% The stationary states of biological systems have a subtle structure, neither “frozen” into a well ordered crystal, nor chaotic and disordered like a gas.
...
We plan to consider two broad compelxity measures: 1) \textit{I-complexity:} ($\mu_\C^1$)  the number of non-trivial models we can infer given the data and an upper bound on temporal memeory, normalized by the maximum number of possible models,   and 2) the \textit{Z-Complexity:}($\mu_\C^2$) which is the average number of states in the set of inferred models.
...
Frequency-domain analysis is a classical tool for linear time-inavraint systems, that has enabled deisgn of sophisticated engineering systems over the last century. Unfortunately, the elegant tools of the frequency domain do not easily generalize to non-linear, mixed variable, multi-scale complexities of social interactions. Leveraging our ability to quantify directional influence propagation between observables as a function of possible time delays, we aim to investigate one possible design of an analogous tool. We cal this the $\gamma(\Delta)$ plot, where $\gamma$ is an iinferred measure of predictive influence, and $\Delta$ is the tiem delay over which the influence transpires.

In our preliminary work, we evaluated the $\gamma(\Delta)$ analytic for a diversity of dyanmical systems, ranging from simple linear, to non-linear to chaotic to simuated and real-world complex systems (See Fig.~\ref{figgamma}).
We concluded that in simple systems, the $\gamma(\Delta)$ plot reveals clear macro-scopic structure corresponding to periodic behaviors and/or near-exponential deacy. Similar behavior is observed in simulated complex systems. In the real-world systems, all obvious macroscopic  structure vanishes | suggesting the emergence of scale-free behavior (there are apparently no specific time or spatial scales that apply to the overall system).

These observations might not be typical, as not all complex systems will have this scale-free behavior. Some systems may have periodicity enforced by  the nature of the dynamics being analyzed. Nevertheless, we will investigate the applicability of the  $\gamma(\Delta)$ plot to analyze non-temporally localized dynamical chracteristics much in the same manner that the frequency domain is used for linear systems.

\subsection*{Measures  of External Influence \& Closed vs Open Systems}
Apart from the question of designing tools to measure computable deviation of simulated systems from their real-world counterparts, we aim to develop tools that
estimate the possibility of external drivers in a complex social system, $i.e.$, analyze if the system is open or closed.
